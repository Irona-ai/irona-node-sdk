...
let PROVIDERS: Record<string, ProviderInfo> = {
  ...
  anthropic: {
    ...
    support_response_model: ["claude-2.1", "claude-3-opus-20240229"],
    openrouter_identifier: {
...
export async function updateProvidersFromGist() {
  try {
    const response = await axios.get(
      "https://gist.githubusercontent.com/tshrjn/f55b3ebd90eda8a0e65bf8435419edff/raw/0a62eb3cee8e45aca1be94ee166939cb9a1455d3/supported_models_pricing.json"
    );
    PROVIDERS = response.data;
    console.info("Providers updated from Gist.");
  } catch (error) {
    console.error(
      "Failed to load providers from Gist. Falling back to local file."
    );
  }
}
