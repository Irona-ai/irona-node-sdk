...
export class IronaImageClient extends IronaChatClient {
  ...
  async generateImage(payload: ImageGenerationPayload) {
    ...
    // For image generation, we use the prompt field directly
  ...
  private getImageModelInstance(provider: string, model: string) {
    ...
    const imageModelFunction = providerImageModels[provider as keyof typeof providerImageModels];
    if (!imageModelFunction) {
      return null; // Provider doesn't support image generation
    }
    
    return (modelName: string) => imageModelFunction(modelName);
  }
